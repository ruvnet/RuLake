//! Streamable HTTP transport for `mcp-ruqu`.
//!
//! v0.1 lifts the transport pattern wholesale from `mcp-rvdna::http`
//! (commit a66d65f / iter 32 CORS layer): stdio + Streamable HTTP, no
//! auth beyond the loopback default + an opt-in for non-loopback
//! `--auth none` via `--insecure-allow-no-auth`. Bearer / JWT / mTLS
//! lift wholesale from `mcp-server::http` in v0.2 — the request /
//! response shape here is deliberately the same so that lift is
//! mechanical.
//!
//! The CORS layer is byte-identical to the rvdna lift: echo the
//! requesting Origin (or `*` if absent), expose the MCP-specific
//! session / version headers, and short-circuit OPTIONS preflight.
//! Iter 41's cross-component test (Console at :4173 → mcp-ruqu at
//! :17442) rides on the same five preflight assertions.
...
use crate::server::RuquMcpServer;

/// Auth mode selected at startup. v0.1 ships only `None` — the rest
/// of the variants exist so the wire signature is stable when the
/// JWT/mTLS lift from mcp-server lands in v0.2.
...
/// Stub bearer-auth carrier — token comparison itself lifts in v0.2.
...
pub async fn serve(
    ...
    if matches!(auth, AuthMode::None) && !is_loopback && !insecure_allow_no_auth.0 {
        anyhow::bail!(
            "ruqu-mcp: --auth none on a non-loopback bind ({bind}) requires \
    ...
    // Reverse-proxy / managed-runtime support (Cloud Run et al.) —
    // same pattern as mcp-server commit 4248b75 / mcp-rvdna sibling.
    ...
    tracing::info!(%bind, "ruqu-mcp http listening");
        ...
        tokio::spawn(async move {
            ...
            let h_svc = service_fn(move |req: Request<Incoming>| {
                ...
                async move {
                    ...
                    // and short-circuit OPTIONS preflight. Lifted from
                    // mcp-rvdna's iter 32 fix; v0.2 will tighten to an
            ...
            if let Err(e) = auto::Builder::new(TokioExecutor::new())
                ...
                .await
            {
                tracing::debug!(target: "ruvector_rulake_mcp_ruqu::http", error = %e, "ruqu-mcp http connection ended");
            }
        });
    }
