...
The fourth is **the bundle contract preservation**. `Bundle(...)` constructs a `RuLakeBundle` via the same `RuLakeBundle::new` (`crates/core/src/bundle.rs:166`) the Python SDK reaches; the witness recipe at `crates/core/src/bundle.rs:362` is unchanged across language boundaries. A Node process and a Python process constructing the same bundle (same `dataRef`, dim, seed, rerank, generation) produce byte-identical witnesses — the witness recipe is purely a function of bytes. `Bundle.fromJson(s)` and `Bundle.readFromDir(dir)` propagate the witness-fail-closed posture from `crates/core/src/bundle.rs:340`-area; a Node caller reading a tampered bundle gets an `RuLakeError` with `code: RULAKE_WITNESS_*`, never silently bad data.
...
**The cache and federation primitives are reachable.** `lake.searchFederated(targets, query, k)` from Node reaches `RuLake::search_federated` at `crates/core/src/lake.rs:521`, which fans out across registered backends in parallel via rayon. A Node caller assembling a 10-shard federated query pays one `memcpy` on the query vector at the FFI boundary and one JS-array allocation on the result; the parallel scan happens in pure Rust on libuv worker threads. The bench gate confirms this stays inside the 1.10× tax budget.

**The bundle and witness contract is reachable.** `new Bundle(...)` from Node constructs a `RuLakeBundle` via `RuLakeBundle::new` (`crates/core/src/bundle.rs:166`), which calls `compute_witness` (`crates/core/src/bundle.rs:362`) to produce the SHAKE-256(32) hex witness. A Node process and a Python process and a Rust process constructing the same bundle produce byte-identical witnesses, because the witness recipe is purely a function of bytes. `Bundle.fromJson` and `Bundle.readFromDir` propagate the witness-fail-closed posture; a Node caller reading a tampered bundle gets a `RuLakeError`, never silently bad data. The DoS hardening (64 KiB body cap, 4 KiB per-field cap, 128-char witness cap from `crates/core/src/bundle.rs:218`-area) is inherited verbatim.
...
- Federation primitive reachable from Node: `crates/core/src/lake.rs:521` (`search_federated`)
